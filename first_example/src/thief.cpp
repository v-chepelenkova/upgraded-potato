...
void Thief::InteractWithInhabitant(Inhabitant* that) {
  ...
  switch (that->GetType()) {
    case PRODUCT:
      ...
      std::cout << product->GetName() << "'s been stolen. Out of "
          << std::to_string(product->GetAmount()) << " items only "
          << std::to_string((product->Add(-1), product->GetAmount())) << " are left" << std::endl;
      break;
    case THIEF:
...
void Thief::Show() const {
  std::cout << "Thief: " << luck * 100 << "% lucky" << std::endl;
}
